...
public class EnrollmentServiceImpl implements EnrollmentService {
  ...
  public EnrollmentDomain createEnrollment(EnrollmentJson enrollmentJson) {
    CourseDomain course = courseRepository
      .findById(enrollmentJson.getCourse_id())
      .orElseThrow(NotFoundException::new);
    StudentDomain student = studentRepository
      .findById(enrollmentJson.getStudent_id())
      .orElseThrow(NotFoundException::new);

    var alreadyRegister = enrollmentRepository.existsByCourseAndStudent(
      course,
      student
    );
    ...
    var enrollmentDomain = EnrollmentDomain.fromEnrollmentService(
      course,
      student
    );
  ...
  @Override
  public Page<EnrollmentDomain> getEnrollment(
    Pageable page,
    ...
    if (course.isPresent() || student.isPresent()) {
      EnrollmentFilter enrollmentFilter = EnrollmentFilter
        .builder()
        .course(course)
        .student(student)
        .build();
  ...
  public void delete(Long Id) {
    var enroll = enrollmentRepository
      .findById(Id)
      .orElseThrow(() -> new NotFoundException());
